...
describe("CDK Snapshots", () => {
  ...
  describe("SecurityStack snapshot", () => {
    ...
    test("WAF AWSManagedRulesCommonRuleSet excludes SizeRestrictions_BODY", () => {
      ...
      const excludedRules =
        managedRule.Statement.ManagedRuleGroupStatement.ExcludedRules;
  ...
  describe("Full stack composition snapshot", () => {
    ...
    test("AuthStack Cognito User Pool references custom auth Lambdas", () => {
      authTemplate.hasResourceProperties("AWS::Cognito::UserPool", {
        ...
        LambdaConfig: {
          DefineAuthChallenge: { "Fn::GetAtt": expect.arrayContaining(["DefineChallenge", "Arn"]) },
          CreateAuthChallenge: { "Fn::GetAtt": expect.arrayContaining(["CreateChallenge", "Arn"]) },
          VerifyAuthChallengeResponse: {
            "Fn::GetAtt": expect.arrayContaining(["VerifyChallenge", "Arn"]),
          },
        },
      });
    });
