...
describe("WhatsForLunch Infrastructure Stacks", () => {
  ...
  test("AuthStack synthesizes with Cognito", () => {
    const dataStack = new DataStack(app, "TestDataForAuth", {
      ...stackProps,
      config,
    });
  ...
  test("OpsStack synthesizes with dashboards + alarms", () => {
    ...
    template.resourceCountIs("AWS::CloudWatch::Alarm", 6); // Lambda, AppSync, DynamoDB, Bedrock, AI cost
    template.resourceCountIs("AWS::SNS::Topic", 2); // Alerts + Critical
  });

  test("AiStack synthesizes with 3 Lambdas", () => {
    const dataStack = new DataStack(app, "TestDataForAi", {
      ...stackProps,
      config,
    });
  ...
  test("NotificationsStack synthesizes with EventBridge", () => {
    const dataStack = new DataStack(app, "TestDataForNotif", {
      ...stackProps,
      config,
    });
  ...
  test("All stacks export required outputs", () => {
    const dataStack = new DataStack(app, "TestDataWithOutputs", {
      ...stackProps,
      config,
    });
    ...
    template.hasOutput("TableName", {
      Export: { Name: { "Fn::Sub": "wfl-TableName-*" } },
    });
    template.hasOutput("PhotosBucketName", {
      Export: { Name: { "Fn::Sub": "wfl-PhotosBucket-*" } },
    });
  });
});
...
describe("Integration: API + Data + Auth stacks", () => {
  test("ApiStack can reference DataStack and AuthStack", () => {
    ...
    const dataStack = new DataStack(app, "IntegrationData", {
      ...stackProps,
      config,
    });
    const authStack = new AuthStack(app, "IntegrationAuth", {
      ...stackProps,
      config,
      dataStack,
    });
    const aiStack = new AiStack(app, "IntegrationAi", {
      ...stackProps,
      config,
      dataStack,
    });
