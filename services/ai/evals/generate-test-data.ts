/**
 * Generate test data for eval datasets.
 * Useful for expanding ground-truth.csv files quickly.
 *
 * Run: npx ts-node generate-test-data.ts
 */

import fs from 'fs';
import path from 'path';

interface ClassifyFoodExample {
  photoPath: string;
  foodType: string;
  daysSafe: number;
...
function generateClassifyFoodExamples(count: number): ClassifyFoodExample[] {
  const examples: ClassifyFoodExample[] = [];

  for (let i = 0; i < count; i++) {
    const food = FOOD_TYPES[i % FOOD_TYPES.length];
    const location = STORAGE_LOCATIONS[i % STORAGE_LOCATIONS.length];

    // Adjust days safe based on storage location
...
function generateOcrDateExamples(count: number): OcrDateExample[] {
  const examples: OcrDateExample[] = [];
  const today = new Date();

  for (let i = 0; i < count; i++) {
    // Generate dates 1-365 days from now
    ...
    const keyword = DATE_KEYWORDS[i % DATE_KEYWORDS.length];
...
function saveClassifyFoodExamples(examples: ClassifyFoodExample[], filename: string) {
...
function saveOcrDateExamples(examples: OcrDateExample[], filename: string) {
...
export function generateAllTestData(classifyCount = 100, ocrCount = 50) {
...
2. Check accuracy against targets:
   - classify-food: >= 90%
   - ocr-expiry-date: >= 95%
  `);
}
...
if (import.meta.url === `file://${process.argv[1]}`) {
  const classifyCount = parseInt(process.argv[2] || '100');
  const ocrCount = parseInt(process.argv[3] || '50');

  generateAllTestData(classifyCount, ocrCount).catch((error) => {
    console.error('Error generating test data:', error);
    process.exit(1);
  });
}

export { generateClassifyFoodExamples, generateOcrDateExamples };
